...
class Scanner:
    ...
    def start_scan(self):
        ...
        """
        try:
            print_module(f"Starting wifi sniffing on interface {self.context.main_interface.dev}")
            self.context.main_logger.info(f"Starting wifi sniffing on interface {self.context.main_interface.dev}")
